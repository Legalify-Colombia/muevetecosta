...
import { Users, GraduationCap, Plane, FileText, FolderOpen, Mail, History, Layout, BarChart3, FileCheck } from 'lucide-react';
...
const AdminSidebar = ({
  ...
  return <aside className="w-64 bg-white border-r border-gray-200 overflow-y-auto">
      <div className="p-6">
        <h2 className="text-xl font-bold text-gray-900">Mi Panel</h2>
      ...
      <nav className="px-4 pb-6">
        <div className="space-y-1">
          ...
          return <Button key={item.id} variant={activeTab === item.id ? "secondary" : "ghost"} className={cn("w-full justify-start h-auto p-3 text-left", activeTab === item.id && "bg-blue-50 text-blue-700 border-blue-200")} onClick={() => onTabChange(item.id)}>
